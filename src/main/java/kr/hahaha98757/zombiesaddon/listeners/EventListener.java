...
public class EventListener {
	...
	@SubscribeEvent
	public void noMods(EntityJoinWorldEvent event) {
		...
		if (ZombiesAddon.isSST && SpawnTimeRendererTransformer.isTransformer()) {
			Minecraft.getMinecraft().thePlayer.addChatComponentMessage(new ChatComponentText(CommandInfo.LINE
					+ "\nYou are using Show Spawn Time.\nWave delay of SST will be off.\n" + CommandInfo.LINE));
		}
		if (ZombiesAddon.isZombiesUtils && ZombiesUtilsTransformer.isTransformer()) {
			Minecraft.getMinecraft().thePlayer.addChatComponentMessage(new ChatComponentText(CommandInfo.LINE
					+ "\nYou are using Zombies Utils.\nTimer of Zombies Utils will be off.\n" + CommandInfo.LINE));
		}
		join = true;
