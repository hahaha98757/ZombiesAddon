	...
	public static int getRound() {
		...
		if (scoreObjective != null
			...
			while (var7.hasNext()) {
				...
				if (practice ? (score.getScorePoints() >= 10 && score.getScorePoints() <= 13)
						: score.getScorePoints() == 13 && (s.contains("Round") || s.contains("\uB77C\uC6B4\uB4DC"))) {
					s = s.replaceAll("\u00A7.", "");
					round = 0;
					try {
		...
		if (scoreObjective != null
			...
			while (var7.hasNext()) {
				...
				if (score.getScorePoints() == 3) {
					...
					String str;
					try {
						str = s.split(":")[1].replaceAll("[^a-zA-Z0-9]", "").trim();
					...
					if (str.equalsIgnoreCase("DeadEnd")) {
						map = 1;
					} else if (str.equalsIgnoreCase("BadBlood")) {
						map = 2;
					} else if (str.equalsIgnoreCase("AlienArcadium")) {
						map = 3;
			...
			if (scoreObjective != null
				...
				while (var7.hasNext()) {
					...
					if (score.getScorePoints() == 10) {
						String name;
						name = s.replaceAll("\u00A7.", "").replaceAll("[^a-zA-Z0-9_]", "");
						try {
							name = name.split(":")[0];
						} catch (Exception var13) {
							return null;
