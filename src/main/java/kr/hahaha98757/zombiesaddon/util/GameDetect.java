...
public class GameDetect {
	...
	public static int isDifficult(int map, int round) {
		Scoreboard scoreboard = Minecraft.getMinecraft().thePlayer.getWorldScoreboard();
		ScoreObjective scoreObjective = scoreboard
				.getObjectiveInDisplaySlot(Scoreboard.getObjectiveDisplaySlotNumber("sidebar"));
		if (scoreObjective != null
	...
	public static String[] playerNameDetect() {
		String[] names = { "", "", "", "" };
		Scoreboard scoreboard = Minecraft.getMinecraft().thePlayer.getWorldScoreboard();
		ScoreObjective scoreObjective = scoreboard
				.getObjectiveInDisplaySlot(Scoreboard.getObjectiveDisplaySlotNumber("sidebar"));
		if (scoreObjective != null
			...
			while (var7.hasNext()) {
				...
				if (score.getScorePoints() == 10) {
					...
					try {
						String s2 = s.split(":")[0].replaceAll("[^a-zA-Z0-9_]", "").trim();
						s = s.split(":")[1].replaceAll("[^a-zA-Z0-9\uAC00-\uD7A3]", "").trim();
						if (s.equalsIgnoreCase("quit") || s.equalsIgnoreCase("\uB5A0\uB0A8")) {
							s2 = "!" + s2;
						}
