...
import kr.hahaha98757.zombiesaddon.tweaker.SpawnTimeRendererTransformer;
import kr.hahaha98757.zombiesaddon.tweaker.ZombiesUtilsTransformer;
...
public class EventListener {
	...
	public void overlayKorean(ClientChatReceivedEvent event) {
		if (!ZombiesAddonConfig.enableMod && !ZombiesAddonConfig.autoDisableMod) {
			return;
		}
	...
	public void oldSST(EntityJoinWorldEvent event) {
		if (!ZombiesAddonConfig.enableMod) {
			return;
		}
	...
	@SubscribeEvent
	public void autoDisableMod(RenderGameOverlayEvent.Post event) {
		if (!ZombiesAddonConfig.autoDisableMod) {
			return;
		}
		
		if (!GameDetect.isZombiesGame()) {
			ZombiesAddonConfig.enableMod = false;
		} else {
			ZombiesAddonConfig.enableMod = true;
		}
	}
}
