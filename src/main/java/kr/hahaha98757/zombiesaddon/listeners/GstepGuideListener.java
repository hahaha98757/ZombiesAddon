...
public class GstepGuideListener {
	...
	public void onRenderLiving(RenderLivingEvent.Post event) {
		...
		if (event.entity instanceof EntityLivingBase) {
			...
			if (!livingEntity.getName().replaceAll("\u00A7.", "").equals("1,000 Gold") && !livingEntity.getName()
					.replaceAll("\u00A7.", "").replaceAll("[^0-9\uAC00-\uD7A3]", "").equals("1000\uACE8\uB4DC")) {
				return;
			}
	...
	private boolean isRender(EntityLivingBase entity) {
		try {
			...
			if (!isPos(entity.posX, getGalleryEntity().posX, 0.3) || !isPos(entity.posY, getGalleryEntity().posY, 0.3)
					|| !isPos(entity.posZ, getGalleryEntity().posZ, 0.3)) {
				return false;
			}
