...
public class ZSVListener {
	...
	public static void recalcActualLines() {
		...
		for (int j = currentLine; j < currentLine + linesOfView; ++j) {
			if (j < START_LINES.size()) {
				String line = (String) START_LINES.get(j);
				if (line.isEmpty()) {
	...
	public static void drawRect(int left, int top, int right, int bottom, int color) {
		...
		buffer.pos((double) left, (double) bottom, 0.0).endVertex();
		buffer.pos((double) right, (double) bottom, 0.0).endVertex();
		buffer.pos((double) right, (double) top, 0.0).endVertex();
		buffer.pos((double) left, (double) top, 0.0).endVertex();
		tessellator.draw();
