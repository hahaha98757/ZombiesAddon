...
public class PowerupAlarmListener {
	...
	private static final String POWERUP_DESPAWN = " \u00A7einsta will despawn soon";
	...
	private static boolean instaRound = true;
	private static boolean maxRound = true;
	private static boolean ssRound = true;
	...
	public void onPowerupText(RenderGameOverlayEvent.Post event) {// insta:1 max:2, dg:3, carpenter:4, ss:5, bg:6
		...
		if (!ZombiesAddonConfig.powerupAlarm) {
			return;
		}
		...
		if (insta) {
		...
		} else if (instaP != 0 && ZombiesAddonConfig.instaRound) {
			...
			if (instaP == 2) {
				for (String str : INSTA_PATTERN1) {
					...
					if (round <= i) {
						if (GameDetect.mapDetect() == 1 && i == 5) {
							patternRound = 8;
							break;
						}
						if (GameDetect.mapDetect() != 3 && i == 20) {
							patternRound = 23;
							break;
						}
						patternRound = i;
				...
				String str = "\u00A7cInsta Kill: " + "\u00A7fRound " + (patternRound != 0 ? patternRound : null);
				FontRenderer fr = Minecraft.getMinecraft().fontRendererObj;
				int stringHeight = fr.FONT_HEIGHT;
				int screenHeight = new ScaledResolution(Minecraft.getMinecraft()).getScaledHeight();
				int y = screenHeight - stringHeight;
				...
				for (String str : INSTA_PATTERN2) {
					int i = Integer.parseInt(str);
					if (round <= i) {
						if (GameDetect.mapDetect() != 3 && i == 15) {
							patternRound = 18;
							break;
						}
						if (GameDetect.mapDetect() == 3 && i == 24) {
							patternRound = 0;
							break;
						}
						patternRound = i;
		...
		} else if (maxP != 0 && ZombiesAddonConfig.maxRound) {
			int round = GameDetect.roundDetect();
			int patternRound = 0;
			...
			if (maxP == 2) {
				for (String str : MAX_PATTERN1) {
					...
					if (round <= i) {
						...
						if (GameDetect.mapDetect() != 3 && i >= 31) {
							patternRound = 0;
							break;
						}
						patternRound = i;
				...
				String str = "\u00A79Max Ammo: " + "\u00A7fRound " + (patternRound != 0 ? patternRound : null);
				FontRenderer fr = Minecraft.getMinecraft().fontRendererObj;
				int stringHeight = fr.FONT_HEIGHT;
				int screenHeight = new ScaledResolution(Minecraft.getMinecraft()).getScaledHeight();
				int y = screenHeight - stringHeight;
				...
				for (String str : MAX_PATTERN2) {
					int i = Integer.parseInt(str);
					if (round <= i) {
						if (GameDetect.mapDetect() != 3 && i >= 32) {
							patternRound = 0;
							break;
						}
						patternRound = i;
		...
		if (ss) {
		...
		} else if (ssP != 0 && GameDetect.mapDetect() == 3 && ZombiesAddonConfig.ssRound) {
			int round = GameDetect.roundDetect();
			int patternRound = 0;
			...
			if (ssP == 5) {
				...
				String str = "\u00A75Shopping Spree: " + "\u00A7fRound " + (patternRound != 0 ? patternRound : null);
				FontRenderer fr = Minecraft.getMinecraft().fontRendererObj;
				int stringHeight = fr.FONT_HEIGHT;
				int screenHeight = new ScaledResolution(Minecraft.getMinecraft()).getScaledHeight();
				int y = screenHeight - stringHeight;
	...
	public void onLivingUpdateEvent(LivingEvent.LivingUpdateEvent livingUpdateEvent) {
		if (!GameDetect.isZombiesGame() || !ZombiesAddonConfig.powerupAlarm) {
			return;
		}
	...
	public void isInsta(EntityLivingBase entity) {
		...
		if (name.equals("INSTAKILL") || name.equals("\uC989\uC2DC\ucc98\uCE58")) {
			...
			for (String str : INSTA_PATTERN1) {
				if (str.equals(String.valueOf(GameDetect.roundDetect()))) {
					instaP = 2;
					break;
				}
			}
