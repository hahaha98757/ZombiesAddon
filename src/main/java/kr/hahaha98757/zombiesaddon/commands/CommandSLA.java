...
public class CommandSLA extends CommandBase {
	...
	public String getCommandUsage(ICommandSender sender) {
		return "\u00A7cUsage: /SLA <de|bb|aa|off|old>";
	}
	...
	public void processCommand(ICommandSender sender, String[] args) {
		...
		if (args.length >= 1) {
			if (!args[0].equals("de") && !args[0].equals("bb") && !args[0].equals("aa") && !args[0].equals("off") && !args[0].equals("old")) {
				...
				if (DisplaySLAOverlayListener.oldZombiesSLA) {
					player.addChatComponentMessage(new ChatComponentText("\u00A7eSLA: Set SLA to \u00A7anew."));
					DisplaySLAOverlayListener.oldZombiesSLA = false;
				} else {
					player.addChatComponentMessage(new ChatComponentText("\u00A7eSLA: Set SLA to \u00A7cold."));
					player.addChatComponentMessage(new ChatComponentText(CommandInfo.LINE + "\n\u00A7rOld SLA range: 50 blocks\nNew SLA range: 40 blocks\nIt is changed new SLA when join other world.(e.g. join or leave server, map etc.)\n\u00A7cIt is not used in Hypixel now.\n" + CommandInfo.LINE));
					DisplaySLAOverlayListener.oldZombiesSLA = true;
				}
			} else {
	...
	public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
		if (args.length == 1) {
			return getListOfStringsMatchingLastWord(args, new String[] { "de", "bb", "aa", "off", "old" });
		}
		return null;
	}
