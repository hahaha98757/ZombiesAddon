	...
	public void processCommand(ICommandSender sender, String[] args) {
		...
		if (args.length == 0) {
			player.addChatComponentMessage(new ChatComponentText(LINE + "\nMod name: " + ZombiesAddon.NAME
					...
					+ "\nMod list: Cornering, Block Alarm, DPS Counter, NOTLAST, Auto Splits, ZSV, SLA, Advanced SLA, Auto Rejoin, Powerup Alarm, Boss Alarm, and Lrod Order.\nInfo: A mod that helps playing hypixel zombies.\nCommand tips: Use /info [Cornering|BlockAlarm|DPSCounter|NOTLAST|AutoSplits|ZSV|SLA|AdvancedSLA|AutoRejoin|PowerupAlarm|BossAlarm|LrodOrder] to view mod's description.\nURL: https://github.com/hahaha98757/ZombiesAddon\n"
					+ LINE));
			return;
		}
		...
		if (args.length == 1) {
			return getListOfStringsMatchingLastWord(args,
					new String[] { "Cornering", "BlockAlarm", "DPSCounter", "NOTLAST", "AutoSplits", "ZSV", "SLA",
							"AdvancedSLA", "AutoRejoin", "PowerupAlarm", "BosAlarm", "LrodOrder" });
		}
		return null;
	}
}
