...
public class PlayerTrackerListener {
	...
	public void onRenderWorldLast(RenderWorldLastEvent event) {
		...
		for (EntityPlayer player : Minecraft.getMinecraft().theWorld.playerEntities) {
			...
			if (player.getName().equals(GameDetect.getPlayerNames()[1])) {
				GL11.glColor4f(0.3F, 1.0F, 0.3F, 1.0F);
			} else if (player.getName().equals(GameDetect.getPlayerNames()[2])) {
				GL11.glColor4f(0.3F, 1.0F, 1.0F, 1.0F);
			} else if (player.getName().equals(GameDetect.getPlayerNames()[3])) {
